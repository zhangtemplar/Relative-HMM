...
%}   
function [model_list, idx, model_score]=relative_hmm_two(data, relative_set, num_state, model, svm_C, gap_likelihood)
    %% ====================================================================
    % constant
    ...
    if nargin<5 || isempty(svm_C)
        svm_C=1000;
    end
    % what is the gap for two comparing pair of loglikelihood
    if nargin<6 || isempty(gap_likelihood)
        gap_likelihood=10;
    end
    ...
    while 1
        ...
        if use_alm
            % use alm algorithm
            x=update_model(x, Aeq, H, f, A_sparse, b, lb, ub, svm_C,...
        ...
        if (iter>=max_iter || abs(now_conv-prev_conv)<tol ||...
