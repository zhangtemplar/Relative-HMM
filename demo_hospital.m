...
[video_data video_name video_label action_size]=...
    dense_load_feature_mat(source_dir, '*_descriptor.mat', {feature_type coordinate_type}, 0.01, 1);
...
relative_set(rand(size(relative_set))<0.8)=0;
%% ========================================================================
% training
t1=tic;
[model_list model_idx model_score]=relative_hmm(data, relative_set, 10);
t1=toc(t1);
%% ========================================================================
% testing
...
save('student_result', 'model', 'data', 'first_week', 'last_week', 'relative_set', 'path', 'log_lik');
% =========================================================================
...
imshow(test_result, 'parent', h);
set(gcf, 'position', [100 100 action_size action_size]);
save('student_result', 'model', 'data', 'first_week', 'last_week', 'relative_set', 'path', 'log_lik');
%% ========================================================================
% training with new model
t2=tic;
[model_list model_idx model_score]=relative_hmm_two(data, relative_set, 10);
t2=toc(t2);
%% ========================================================================
% testing
model=model_list(:, model_idx);
...
for j=1: length(model)
    prior=log(model(j).pi_init);
    transmat=log(model(j).pi);
    obsmat=log(model(j).theta);
    for i=1: length(data)
