...
f1=fopen('simulation_old_result.txt', 'w');
...
for ii=1: length(number_pair_train)
    ...
    % train
    t=tic;
    [model_list model_idx model_score]=relative_hmm(data_train, relative_set, 10);
    time_info(ii)=toc(t);
    fprintf(f1, 'Time %f\t', ii, time_info(ii));
    continue;
    ...
    %% ====================================================================
    % save result
    save(num2str(ii, 'simulation_old_result_%02d'), 'model_list', 'model_idx', 'model_score', 'relative_set',...
        'train_path', 'train_loglik', 'train_result', 'train_path', 'test_loglik', 'test_result');
end
fclose(f1);
sendEmail2Self('Result for improved', sprintf('%f, ', time_info), 'simulation_old_result.txt');
