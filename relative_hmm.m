...
%}   
function [model_list idx model_score]=relative_hmm(data, relative_set, num_state, model, do_scale, svm_C)
    %% ====================================================================
    % constant
    max_iter_init=20;
    max_iter=100;
    % do we normalize the likelihood given the video length
    if nargin<5 || isempty(do_scale)
        do_scale=0;
    end
    % do we require the probability vector sums to 1
    % do_normalize=0;
    % the weight to the slack variables
    if nargin<6 || isempty(svm_C)
        svm_C=1000;
    end
    ...
    while (~converged)
        ...
        for i=1: length(data)
            p_xt_zt{i}=emission_multinomial(data{i}, theta);
